...
class Picking(models.Model):
    ...
    @api.constrains('transfer_by_product_categ', 'categ_id', 'move_lines', 'move_ids_without_package', 'move_line_ids',
                    'move_line_ids_without_package')
    def _check_moves_categ_unicity(self):
        """ Check that all the selected products have the same category"""
        acceptable_categories = self.env["product.category"].search([('id','child_of', self.categ_id.id)]).ids
        if self.transfer_by_product_categ and self.categ_id and self.move_lines:
            ...
            if move_lines_categ_id.filtered(lambda mlc:mlc.id not in acceptable_categories):
            ...
            #    raise UserError(_("All the products must belongs to the same category!"))
            if move_line_ids_categ_id.filtered(lambda mlc:mlc.id not in acceptable_categories):
                raise UserError(_("All the products must belongs to the picking category %s!") % self.categ_id.name)
            ...
            #    raise UserError(_("All the products must belongs to the same category!"))
            if move_line_ids_without_package_categ_id.filtered(lambda mlc:mlc.id not in acceptable_categories):
                raise UserError(_("All the products must belongs to the picking category %s!") % self.categ_id.name)
